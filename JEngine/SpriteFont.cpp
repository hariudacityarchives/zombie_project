...
SpriteFont::SpriteFont(const char *font, int size, char cs, char ce) {
  ...
  for (char c = cs; c <= ce; c++) {
    TTF_GlyphMetrics(f, c, &glyphRects[i].x, &glyphRects[i].z, &glyphRects[i].y,
                     &glyphRects[i].w, &advance);
  ...
  if (!bestPartition) {
    fprintf(stderr,
            "Failed to Map TTF font %s to texture. Try lowering resolution.\n",
            font);
    fflush(stderr);
    throw 282;
  }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bestWidth, bestHeight, 0, GL_RGBA,
  ...
  for (int ri = 0; ri < bestRows; ri++) {
    ...
    for (int ci = 0; ci < bestPartition[ri].size(); ci++) {
      ...
      SDL_Surface *glyphSurface =
          TTF_RenderGlyph_Blended(f, (char)(cs + gi), fg);
      ...
      glTexSubImage2D(GL_TEXTURE_2D, 0, lx,
                      bestHeight - ly - 1 - glyphSurface->h, glyphSurface->w,
                      glyphSurface->h, GL_BGRA, GL_UNSIGNED_BYTE,
                      glyphSurface->pixels);
      glyphRects[gi].x = lx;
      glyphRects[gi].y = ly;
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, rs, rs, GL_RGBA, GL_UNSIGNED_BYTE,
                  pureWhiteSquare);
  ...
  for (i = 0; i < _regLength; i++) {
    ...
    _glyphs[i].uvRect = glm::vec4((float)glyphRects[i].x / (float)bestWidth,
  ...
  _glyphs[_regLength].uvRect = glm::vec4(0, 0, (float)rs / (float)bestWidth,
...
std::vector<int> *SpriteFont::createRows(glm::ivec4 *rects, int rectsLength,
